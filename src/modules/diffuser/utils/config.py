...
def import_class(_class):
    ...
    repo_name0 = __name__.split('.')[0]
    repo_name1 = __name__.split('.')[1]
    ## eg, 'utils'
    module_name = '.'.join(_class.split('.')[:-1])
    ## eg, 'Renderer'
    class_name = _class.split('.')[-1]
    ## eg, 'diffusion.utils'
    module = importlib.import_module(f'{repo_name0}.{repo_name1}.{module_name}')
    ## eg, diffusion.utils.Renderer
    _class = getattr(module, class_name)
    print(f'[ utils/config ] Imported {repo_name0}.{repo_name1}.{module_name}:{class_name}')
    return _class
